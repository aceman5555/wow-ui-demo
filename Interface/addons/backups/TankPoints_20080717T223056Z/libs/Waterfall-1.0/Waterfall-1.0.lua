...
local MINOR_VERSION = "$Revision: 39176 $"
...
local function buildAceOptionsTree(tree, status, options, numLevels, path, rootHandler)
	...
	assert(args,"Options table args is nil")
	local root
	...
	if paneid == "." then
		...
		for k in paneid:gmatch("[^\.]+") do
			...
			if t.args and t.args[k] then
				t = t.args[k]
				if crumbs then
					crumbs = crumbs.." -> "..(t.guiName or t.name or "")
				else
					crumbs = (t.guiName or t.name or "")
				end
			else
				Waterfall:error("Invalid path \"%s\" for Aceoptions table",path)
			end
		end			
		return crumbs
	end
	...
	for i, v in ipairs(feedtmp) do
		...
		if not hidden then
					...
					if v.multiToggle then
						...
						for key, value in pairs(v.validate) do
							...
							if passGet and v.get == nil then
								...
								addFunction(tmpargs,"get",v.get,v.handler or handler,key,v.passValue)
							end
							if passSet and v.set == nil then
								addFunction(tmpargs,"set",passSet,v.handler or handler,v.passValue or feedkeys[v],key)
							else
								addFunction(tmpargs,"set",v.set,v.handler or handler,key,v.passValue)
							end
			...
			elseif v.type == "execute" then
				...
				tinsert(tmpargs, not not v.confirm)
				tinsert(tmpargs,"tooltipTitle")
				tinsert(tmpargs,name)
				tinsert(tmpargs,"tooltipText")
				tinsert(tmpargs,v.desc or false)
...
function WaterfallFrame.prototype:SetSelected(id)
	self.treeview:SetSelected(id)
end
...
local function Linklabel_OnClick(this)
	...
	if func then
		if this.obj.confirm then
			confirmPopup(("Are you sure you want to %s"):format(this.obj.frame:GetText()),func,getArgs(this.obj,"linkArg",1))
		else
...
local function Button_OnClick(this)
	...
	if func then
		if this.obj.confirm then
			confirmPopup(("Are you sure you want to %s"):format(this.obj.frame:GetText()),func,getArgs(this.obj,"execArg",1))
		else
...
function WaterfallDropdown.prototype:CreateLine()
...
function WaterfallTreeView.prototype:Refresh()
	self.noFixScroll = true
	self:Clear()
	if not self.contents then return end
	if self.contentsFunc then
...
function WaterfallTreeView.prototype:AddSection(index,line)
	...
	for i, item in ipairs(line) do
		if not item.hidden then
			...
			local itemdisabled = checkNegatable(line.disabled,handler)
...
function WaterfallTreeView.prototype:SetSelected(lineid)
