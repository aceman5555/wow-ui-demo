...
Revision: $Revision: 38404 $
Author: Whitetooth@Cenarius (hotdogee@bahamut.twbbs.org)
LastUpdate: $Date: 2007-06-04 13:48:37 +0800 (星期一, 04 六月 2007) $
Website:
Documentation:
...
local MINOR_VERSION = tonumber(("$Revision: 38404 $"):sub(12, -3))
...
PatternLocale.zhTW = {
	...
	["DeepScanWordSeparators"] = {
		"及", "和", "並", "，" -- [發光的暗影卓奈石] ID:25894 "+24 攻擊強度及略微提高奔跑速度", [刺客的火焰蛋白石] ID:30565 "致命一擊等級 +6 及閃躲等級 +5"
	},
	["DeepScanPatterns"] = {
		"^(.-)提高最多([%d%.]+)點(.-)$", -- 
		"^(.-)提高最多([%d%.]+)(.-)$", -- 
...
DisplayLocale.zhTW = {
	...
	["StatIDToName"] = {
		...
		["FERAL_AP"] = {"Feral "..ATTACK_POWER_TOOLTIP, "野性強度"},
		["AP_UNDEAD"] = {ATTACK_POWER_TOOLTIP.." (不死生物)", "攻擊強度(不死)"},
		["AP_DEMON"] = {ATTACK_POWER_TOOLTIP.." (惡魔)", "攻擊強度(惡魔)"},

		["HEAL"] = {"法術治療", "治療"},

		["SPELL_DMG"] = {"法術傷害", "法傷"},
		["SPELL_DMG_UNDEAD"] = {"法術傷害(不死)", PLAYERSTAT_SPELL_COMBAT.."法傷".."(不死)"},
		["SPELL_DMG_DEMON"] = {"法術傷害(惡魔)", PLAYERSTAT_SPELL_COMBAT.."法傷".."(不死)"},
		["HOLY_SPELL_DMG"] = {"神聖法術傷害", SPELL_SCHOOL1_CAP.."法傷"},
		["FIRE_SPELL_DMG"] = {"火焰法術傷害", SPELL_SCHOOL2_CAP.."法傷"},
		["NATURE_SPELL_DMG"] = {"自然法術傷害", SPELL_SCHOOL3_CAP.."法傷"},
		["FROST_SPELL_DMG"] = {"冰霜法術傷害", SPELL_SCHOOL4_CAP.."法傷"},
		["SHADOW_SPELL_DMG"] = {"暗影法術傷害", SPELL_SCHOOL5_CAP.."法傷"},
		["ARCANE_SPELL_DMG"] = {"秘法法術傷害", SPELL_SCHOOL6_CAP.."法傷"},
		...
		["MOD_INT"] = {"怪物智力(%)", "怪物智力(%)"},
		["MOD_SPI"] = {"怪物精神(%)", "怪物精神(%)"},
		["MOD_HEALTH"] = {"怪物生命(%)", "怪物生命(%)"},
		["MOD_MANA"] = {"怪物法力(%)", "怪物法力(%)"},
		["MOD_ARMOR"] = {"怪物裝甲(%)", "怪物裝甲(%)"},
		["MOD_BLOCK_VALUE"] = {"怪物格擋值(%)", "怪物格擋值(%)"},
		["MOD_DMG"] = {"怪物傷害(%)", "怪物傷害(%)"},
		["MOD_DMG_TAKEN"] = {"怪物受傷害(%)", "怪物受傷害(%)"},
		["MOD_CRIT_DAMAGE"] = {"怪物致命(%)", "怪物致命(%)"},
		["MOD_CRIT_DAMAGE_TAKEN"] = {"怪物受致命(%)", "怪物受致命(%)"},
		["MOD_THREAT"] = {"怪物威脅(%)", "怪物威脅(%)"},
		["MOD_AP"] = {"怪物攻擊強度(%)", "怪物攻擊強度(%)"},
		["MOD_RANGED_AP"] = {"怪物遠程攻擊強度(%)", "怪物遠攻強度(%)"},
		["MOD_SPELL_DMG"] = {"怪物法術傷害(%)", "怪物法傷(%)"},
		["MOD_HEALING"] = {"怪物法術治療(%)", "怪物治療(%)"},
		["MOD_CAST_TIME"] = {"怪物施法時間(%)", "怪物施法時間(%)"},
		["MOD_MANA_COST"] = {"怪物法力成本(%)", "怪物法力成本(%)"},
		["MOD_RAGE_COST"] = {"怪物怒氣成本(%)", "怪物怒氣成本(%)"},
		["MOD_ENERGY_COST"] = {"怪物能量成本(%)", "怪物能量成本(%)"},
		["MOD_COOLDOWN"] = {"怪物技能冷卻(%)", "怪物技能冷卻(%)"},
...
PatternLocale.deDE = {
	...
	["StatIDLookup"] = {
		...
		["Erhöht durch Arlkanzauber und Arkaneffekte zugefügten Schaden"] = {"ARCANE_SPELL_DMG",},
		...
		["Increases damage done to Undead by magical spells and effects"] = {"DMG_UNDEAD"},
		...
		["Erh\195\182ht Eure Parierwertung"] = {"PARRY_RATING",}, -- ErhÃ¶ht Eure Parierwertung
...
local RatingBase = {
	...
	[CR_HASTE_SPELL] = 40 / 3, -- changed in 2.1.0
...
function StatLogic:GetEffectFromRating(rating, id, level)
	...
	if type(rating) ~= "number" or id < 1 or id > 23 then return 0 end
								...
								for _, pattern in ipairs(L.DeepScanPatterns) do
									...
									if value then
										...
										elseif idTable then
											...
											-- pattern match but not found in L.StatIDLookup, keep looking
											print("  DeepScan Lookup Fail: |cffffd4d4'"..statText.."'|r, pattern = |cff72ff59'"..pattern.."'")
										end
									end
								end -- for
							end
