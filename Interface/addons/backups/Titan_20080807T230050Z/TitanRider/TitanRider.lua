...
function TitanPanelRiderButton_GetButtonText(id)
	...
	if UnitOnTaxi("player") == 1 then
		TitanSetVar(TITAN_RIDER_ID,"PlayerState",4);
...
function TitanRider_PlayerMounted()
	...
	for i = 1,40 do
		local _,_,buff_texture = UnitBuff("player", i)
		local _,_,debuff_texture = UnitDebuff("player", i)
...
function TitanRider_NewPlayerMounted()
-- [[
	local playerClass = UnitClass("player");
	TITAN_DOUBLE_CHECK = false;
...
function TitanRider_GetBuffName(buffIndex)
	...
	TitanPanelTooltip:SetUnitBuff("player", buffIndex);
    local Bname = TitanPanelTooltipTextLeft1:GetText();
