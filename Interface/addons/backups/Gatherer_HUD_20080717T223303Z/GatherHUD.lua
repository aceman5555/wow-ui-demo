--[[
	...
	Revision: $Id: GatherHUD.lua 687 2008-02-03 09:36:32Z Norganna $
...
function lib.Render()
	...
	if ( minimapRotationEnabled ) then
		...
		facing = MiniMapPlayerArrow:GetFacing();
	end
...
function Gatherer_HUD.RunUpdate(delay)
	...
	elseif (rate > 10) then interval = rate/2
	elseif (rate > 5) then interval = 1
	elseif (rate > 3) then interval = 0.5
	elseif (rate > 2) then interval = 0.1
	else interval = 0.001 end
