...
function SW_SelOpt(optButton)
	...
	--2.1.3 added support for the slider
	getglobal(cont.."_ScrollSlider"):SetValue(1);
...
function SW_SetOptTxt(opt)
	...
	if bS == nil then 
		opt.NormalText:SetText(opt.optID);
		opt.HighlightText:SetText(opt.optID);
		return; 
	end
	local oc = bS["OC"];
	local txt = "";
...
function SW_ToggleRunning(doCollection)
	if SW_SYNC_DO and SW_SYNC_TO_USE and #SW_SyncState.peerInfo > 1 then
		doCollection = true;
	end
...
function SW_LockFrames(isLocked)
	if isLocked then
		...
		if not SW_Settings.SW_HideResizer then
...
function SW_SyncList_ScrollUpdate()
	...
	FauxScrollFrame_Update(SW_SyncList,#SW_SyncState.peerInfo,10,20);
	for line=1,10 do
		tmpLine = getglobal("SW_BarSyncFrame_Item"..line);
		lineplusoffset = line + FauxScrollFrame_GetOffset(SW_SyncList);
		if lineplusoffset <= #SW_SyncState.peerInfo then
...
function SW_UpdateBars()
	...
	if SW_BarFrame1_ScrollSlider:IsVisible() then
		if SW_ViewBuffer.currentIndex <= SW_MaxVisRows then
			SW_BarFrame1_ScrollSlider:Hide();
			SW_BarFrame1_ScrollSlider:SetValue(1);
			if SW_Settings.OptLayoutMode == "RIGHT" then
		...
		if SW_ViewBuffer.currentIndex > SW_MaxVisRows then
			SW_BarFrame1_ScrollSlider:Show();
			if SW_Settings.OptLayoutMode == "RIGHT" then
	...
	for i, v in ipairs(SW_Bars["SW_BarFrame1"]) do
		...
		if v.canBeSeen then
			oneEntry = data[i + SW_ViewBuffer.scrollOffset];
			
			if not oneEntry or i + SW_ViewBuffer.scrollOffset > datan or oneEntry.sortOrder == 100 or oneEntry.val == 0 then 
				...
				if bSet.ShowRank then
					v:SetBarText(i + SW_ViewBuffer.scrollOffset.."   "..oneEntry.txt);
				else
				...
				v:SetValue( oneEntry.val /(SW_ViewBuffer.maxVals[oneEntry.sortOrder] / 100));
...
function SW_SendReport(caller, sVar)
	...
	local data = SW_ViewBuffer.data;
	local oneEntry = data[1 + SW_ViewBuffer.scrollOffset];
	...
	local outStr=" -------- ";
	if SW_Settings.RE_UseFrameTxt and bSet.OTF~=nil then
		outStr = outStr..bSet.OTF.." ";
	else
		outStr = outStr..inf.t.." ";
	end
	...
	local datan = #data;
	for i=1 + SW_ViewBuffer.scrollOffset, SW_Settings.ReportAmount + SW_ViewBuffer.scrollOffset do
		oneEntry = data[i];
		if not oneEntry or i > datan or oneEntry.sortOrder == 100 or oneEntry.val == 0 then 
			break;
		else
...
-- anchorButton must be number 1
function SW_OptUpdateLayout(anchorButton)
	local tmpButton, baseName, nextNewRow, lastRowStartButton;
	SW_RedoOptLayout = false;
	
	if not SW_Settings.OptLayoutMode then
		SW_Settings.OptLayoutMode = "BOTTOM";	
	end
	lastRowStartButton = anchorButton;
	nextNewRow = false;
	_,_, baseName = string.find(anchorButton:GetName(),"(.+)%d+$");
	
	if SW_Settings.OptLayoutMode == "BOTTOM" then
		anchorButton:SetPoint("TOPLEFT", "SW_OptChk_Running", "TOPRIGHT", SW_OPT_SPACER, 0);
	elseif SW_Settings.OptLayoutMode == "RIGHT" then
		--2.1.3 support for the slider
		if SW_BarFrame1_ScrollSlider:IsVisible() then
			anchorButton:SetPoint("TOPLEFT", "SW_BarFrame1", "TOPRIGHT", SW_BarFrame1_ScrollSlider:GetWidth()-SW_OPT_SPACER, -SW_OPT_SPACER);
		else
			anchorButton:SetPoint("TOPLEFT", "SW_BarFrame1", "TOPRIGHT", 0, -SW_OPT_SPACER);
		end
	end
	for i=2, SW_OPT_COUNT do
		tmpButton = getglobal(baseName..i);
		
		if SW_Settings.OptLayoutMode == "RIGHT" then
			if nextNewRow then
				tmpButton:SetPoint("TOPLEFT", lastRowStartButton:GetName(), "TOPRIGHT", SW_OPT_SPACER, 0);
				lastRowStartButton = tmpButton;
			else
				tmpButton:SetPoint("TOPLEFT", baseName..(i-1), "BOTTOMLEFT", 0, -SW_OPT_SPACER);
			end
			if not tmpButton:GetBottom() then
				SW_RedoOptLayout = true;
				return;
			end
			if tmpButton:GetBottom() - SW_OPT_SPACER - tmpButton:GetHeight() < SW_BarFrame1:GetBottom() then
				nextNewRow = true;
			else
				nextNewRow = false;
			end
			...
			if nextNewRow then
				tmpButton:SetPoint("TOPLEFT", lastRowStartButton:GetName(), "BOTTOMLEFT", 0, -SW_OPT_SPACER);
				lastRowStartButton = tmpButton;
			else
				tmpButton:SetPoint("TOPLEFT", baseName..(i-1), "TOPRIGHT", SW_OPT_SPACER, 0);
			end
			if not tmpButton:GetRight() then
				SW_RedoOptLayout = true;
				return;
			end
			if tmpButton:GetRight() + SW_OPT_SPACER + tmpButton:GetWidth() > SW_BarFrame1:GetRight() then
				nextNewRow = true;
			else
				nextNewRow = false;
			end
