...
--		"HOLYCRIT", 	-- chance to crit with holy spells
		...
		"TOHIT",	-- hit rating
		"RANGEDHIT",	-- ranged hit rating (+30 rating hunter scope)
...
function BonusScanner:ScanLine(line)
	...
	-- any other line (standard stats, enchantment, set name, etc.)
		...
		--jmlstele: Remove colouring from enchants
		if (string.sub(line,0,10) == "|cffffffff") then
			newline = string.sub(line,11,-4);
			line = newline
		end
...
function BonusScanner:PrintInfoDetailed()
	...
	for i, bonus in pairs(BonusScanner.types) do
		if(BonusScanner.bonuses[bonus]) then
			first = true;
			s = "(";
