--[[
	...
	Revision: $Id: LibSwag.lua 495 2007-02-26 01:56:07Z esamynn $
...
if (create or update) then
	...
	function myLib.OnEvent(event, a1,a2)
	...
	function myLib.Spell(event, caster, spell, rank, target)
		...
		if (event == "UNIT_SPELLCAST_SENT") then
			...
			LibSwag.tracker.tooltip = LibSwag.GetLastTip()
			isCasting = true
	...
	function myLib.RecordLoot(ltype, method)
		...
		for i = 1, count do
			...
			if (not lLink and LootSlotIsCoin(i)) then
				local i,j,val
				i,j, val = string.find(lName, "(%d+) "..COPPER)
				if (i) then coin = coin + val end
				i,j, val = string.find(lName, "(%d+) "..SILVER)
				if (i) then coin = coin + (val*100) end
				i,j, val = string.find(lName, "(%d+) "..GOLD)
	...
	function myLib.SetTooltip(line, newtext, r,g,b,a)
		if (not line) then
			...
			for i=1, lines do
				...
				log("SetTooltip", N_DEBUG, "Set Tooltip Line", "Setting tooltip line", i, "=", gtext)
		...
		if (LibSwag.lastTipTime and text ~= LibSwag.lastTipText) then
			...
			if (delta < 0.8) then
				log("SetTooltip", N_DEBUG, "Discard Tooltip", "Discarding tooltip because we have one that's only ", delta, "seconds old")
				return
			end
		end

		-- We're not interested in unit tooltips or any interface tips
		if (not UnitName("mouseover") and GetMouseFocus() == WorldFrame) then
			LibSwag.lastTipText = text
			LibSwag.lastTipTime = GetTime()
			log("SetTooltip", N_DEBUG, "Tooltup Text + Time", "Found usable tooltip text =", text, " and time =", LibSwag.lastTipTime)
		else
			log("SetTooltip", N_DEBUG, "Not interested", "Current mouseover is not a doodad. UnitName =", UnitName("mouseover"), " and focus =", (GetMouseFocus() and GetMouseFocus():GetName() or "nil"))
		end
	end
	...
	function myLib.GetLastTip()
		...
		if (not LibSwag.lastTipTime or delta > 10) then
			log("GetLastTip", N_DEBUG, "Remove Last Tooltip", "Removing ancient tooltip data: text = ", LibSwag.lastTipText, "age =", delta)
			LibSwag.lastTipTime = nil
			LibSwag.lastTipText = nil
			return nil
		end
		log("GetLastTip", N_DEBUG, "Get Last Tooltip", "Return last text =", LibSwag.lastTipText, "and time =", LibSwag.lastTipTime)
		return {
	...
	if (not LibSwagFrame) then
		...
		function LibSwagFrame.OnEvent() LibSwag.OnEvent(event, arg1, arg2) end
