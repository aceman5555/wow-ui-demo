...
-- this is a bogus global, it will nevet get set because Titan Vars wont have time to load
...
function TitanPanelRiderButton_OnLoad()
     this.registry = { 
          ...
          savedVariables = {
               ...
               ShowState = 1,
               PlayerState = 1,
               RiderItems = {"FeetSlot","Trinket0Slot","HandsSlot","Trinket0Slot","Trinket0Slot","Trinket0Slot","FeetSlot"},
               RiderEquip = {"Titan Nil","Titan Nil","Titan Nil","Titan Nil","Titan Nil","Titan Nil","Titan Nil"}
          }
     };
...
function TitanPanelRightClickMenu_PrepareRiderMenu()
	...
	if ( UIDROPDOWNMENU_MENU_LEVEL == 2 ) and ( UIDROPDOWNMENU_MENU_VALUE == TITAN_RIDER_OPTIONS_EQUIP ) then
		...
		local i;
		for i = 1, table.getn(TITAN_RIDER_ITEM_DESCS) do
			if TITAN_RIDER_ITEM_DESCS[i]~=nil then
				info = {};
				info.text = TITAN_RIDER_ITEM_DESCS[i];
				info.value = TITAN_RIDER_ITEM_DESCS[i];
...
for i = 1, table.getn(TITAN_RIDER_ITEM_DESCS) do
 if TITAN_RIDER_ITEM_DESCS[i]~=nil then
  if TITAN_RIDER_ITEM_DESCS[i] == RiderItem then     
                                        ...
                                        if  itemName == TITAN_RIDER_ITEM_NAMES[EquipGear] then
                                             ...
                                             for f = 1, 30 do
                                                  ...
                                                  if (field ~= nil) then
                                                       ...
                                                       if text ~=nil and TITAN_RIDER_ITEM_DESCS[EquipGear]~= nil then
