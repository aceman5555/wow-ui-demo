...
--[[	["player1"] = "charactername",
		...
		["player3"] = "charactername",
		["player4"] = "charactername",
		["player5"] = "charactername",	]]
	},
	["Rate"] = {
		["Raid"] = {
			["Name"] = "Raid",
...
do
	...
	function OpenRaidToggleWordInvite()
		if OpenRaidFrameInvitePlusInvite:GetText() == L["Use whisper invite"] then
			...
			OpenRaidFrame:RegisterEvent("CHAT_MSG_BN_WHISPER")
		else
			OpenRaidFrame:UnregisterEvent("CHAT_MSG_BN_WHISPER")
...
do
	...
	function OpenRaidRemoveFriends()
		...
		if not _G["OpenRaidRemoveFriendsFrame"] then
			...
			local ScrollFrame = CreateFrame("ScrollFrame", "OpenRaidRemoveFriendsScrollFrame", OpenRaidRemoveFriendsFrame, "FauxScrollFrameTemplate");
...
do
	...
	local function GroupFontString(self, n, Target, i)
		...
		if not self["OpenRaidGroupFrameCheckbox" .. i] then
			...
			i==1 and "TOPLEFT" or "BOTTOMLEFT",i==1 and 5 or 0,i==1 and -5 or 15);
			cb:SetScript("OnClick", function(self, button, down)
				OR_db.Group["player" .. i] = self:GetChecked() and OpenRaidFrame["OpenRaidGroupFrameFontString" .. i]:GetText() or nil; --We should rate/not rate this person
		...
		if not self["OpenRaidGroupFrameFontString" .. i] then
			...
			i==1 and "TOPLEFT" or "BOTTOMLEFT", i==1 and 20 or 0, i==1 and -7 -5, DEFAULT, nil)
		end
	...
	local function GroupUpdate()
		...
		if IsInRaid(LE_PARTY_CATEGORY_HOME) and (select(2, GetInstanceInfo()) == "none" or select(2, GetInstanceInfo()) == "raid") then --Apparently IsInRaid() returns true in arenas
			...
			if GetNumGroupMembers() > LastRaidSize then
		...
		elseif IsInGroup(LE_PARTY_CATEGORY_HOME) then
			...
			if GetNumSubgroupMembers() > LastRaidSize then
				LastRaidSize = GetNumSubgroupMembers()
			end
