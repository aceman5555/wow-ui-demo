...
function LootCouncilBot_OnEvent(event)
	...
	if (event == "PLAYER_TARGET_CHANGED") then
		...
		if (target ~= nil) then
			if (strfind(strlower(target), "vyragosa") or strfind(strlower(target), "time-lost")) then
				for i=1,5 do
				SendChatMessage("Rare elite detected! Found Target: " .. target, "WHISPER", nil, UnitName("player"));
				end
			end
		end
